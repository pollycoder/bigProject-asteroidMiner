...
if X(8) < rpMinNew || X(9) < rpMinNew || X(10) < rpMinNew
    J = penalty;
    return
end
...
% GA-1: SOI (t1)
[vt121New, vt122New, ~] = SOI_opt(vt11New, vt13New, vEt1New, muEarthNew, X(8), X(11));

% GA-2: SOI (t2)
[vt221New, vt222New, ~] = SOI_opt(vt21New, vt23New, vMt2New, muMarsNew, X(9), X(12));
...
[vt4New, vt51New] = LambSol(rAt4New, rMt5New, ...
                           tAMNew, muSunNew);               % Lambert problem 4: A->M

% Arrival: M-->E (t5-t6)
tEM2New = X(7) - X(6);
[rEt6New, vEt6New] = rv02rvf(rE0New, vE0New, ...
                             X(7), muSunNew);               % RV of Mars (t=t6)
[vt53New, vt6New] = LambSol(rMt5New, rEt6New, ...
                             tEM2New, muSunNew);            % Lambert problem 5: M->E
...
% Impulse 7-1 7-2: t6
dvt6New = vEt6New - vt6New;
dvt6NormNew = norm(dvt6New);
dvt6Norm = dvt6NormNew / vUnit;
if dvt6Norm < 4
    dvt6New = zeros(3, 1);
    dvt6Norm = 0;
    dvt6NormNew = 0;
else
    dvt6Vector = dvt6New / dvt6NormNew;
    dvt6New = dvt6New - 4 * dvt6Vector;
    dvt6Norm = dvt6Norm - 4;
    dvt6NormNew = dvt6Norm * vUnit;
end
...
dvAfterSampNew = dvt4NormNew + dvt51NormNew + dvt52NormNew + dvt6NormNew;
